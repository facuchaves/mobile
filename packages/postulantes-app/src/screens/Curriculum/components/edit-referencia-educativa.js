...
const EditReferenciaEducativa = props => {
  // eslint-disable-next-line react/prop-types
  const { estudioRelacion, componentId, data, updateCv } = props
  ...
  const deleteReferenciaEducativa = async id => {
    await updateCv(
      CurriculumService.deleteReferenciaEducativa(id).then(response => {
  ...
  const saveReferenciaEducativa = async (values, id) => {
    const hasError = await updateCv(CurriculumService.guardarReferenciaEducativa(values, id))
      ...
      return (
        <ButtonUi
          ...
          onPress={() => {
            Modal(
              null,
              i18n.t('curriculum:referencia_estudio:warning_delete'),
              i18n.t('button:no'),
              null,
              i18n.t('button:si'),
              () => {
                deleteReferenciaEducativa(id)
              }
            )
          }}
        />
  ...
  const { relacion, email, telefono, nombre, apellido, id, referenciaId } = data

  const initialValues = {
    nombre,
    apellido,
    relacion,
    estudioId: referenciaId, // id
    email,
  ...
  return (
    <Content style={styles.container}>
      <HeaderBackScreen
        title={i18n.t('curriculum:referencia_estudio:title')}
        action={() => dismissModal(componentId)}
      />
      <Text style={styles.camposObligatorios}>
        <Text style={styles.asterisco}> *</Text>
        {i18n.t('validation:campos_obligatorio')}
      </Text>
      <Formik
        initialValues={initialValues}
        onSubmit={values => {
          saveReferenciaEducativa(values, id)
        }}
        ...
        {({ values, errors, setFieldTouched, setFieldValue, isValid, handleSubmit }) => (
          <Form>
            <Content style={styles.bloq1}>
              {/* Nombre */}
              <InputUi
                label={i18n.t('curriculum:referencia_estudio:edit_form:nombre')}
                required
                error={!!errors.nombre}
                value={values.nombre}
              ...
              {/* Apellido */}
              <InputUi
                label={i18n.t('curriculum:referencia_estudio:edit_form:apellido')}
                required
              ...
              {/* Email */}
              <InputUi
                label={i18n.t('curriculum:referencia_estudio:edit_form:email')}
                required
                error={!!errors.email}
                value={values.email}
                setFieldValue={itemValue => {
              ...
              <Item stackedLabel style={styles.item}>
                <Label style={styles.label}>
                  {i18n.t('curriculum:referencia_estudio:edit_form:relacion')}
                  <Text style={styles.asterisco}> *</Text>
                </Label>
                <View style={[styles.containerPicker, !!errors.relacion && { borderWidth: 1, borderColor: 'red' }]}>
                  ...
                  >
                    ...
                    <Picker.Item label="Compañera/o" value="companero" />
              ...
              <Item stackedLabel style={styles.item}>
                <Label style={styles.label}>
                  {i18n.t('curriculum:referencia_estudio:edit_form:experiencia_relacion')}{' '}
                  <Text style={styles.asterisco}> *</Text>
                </Label>
                <View style={[styles.containerPicker, !!errors.relacion && { borderWidth: 1, borderColor: 'red' }]}>
                  <Picker
                    ...
                    iosIcon={<Icon name="arrow-down" />}
                    style={styles.inputPicker}
                    placeholder="Experincia"
                    placeholderStyle={styles.PickerTextPlaceholder}
                    placeholderIconColor="#007aff"
                    selectedValue={values.estudioId}
                    onValueChange={async itemValue => {
                      setFieldValue('estudioId', itemValue)
                      setFieldTouched('estudioId')
                    }}
                  >
                    <Picker.Item label={i18n.t('curriculum:referencia_laboral:edit_form:select_option')} value={null} />

                    {estudioRelacion.map(item => {
              ...
              {/* ERROR MENSSAGE */}
              {errors.estudioId && <Text style={styles.errorMenssage}>{errors.estudioId}</Text>}
            </Content>
...
export default EditReferenciaEducativa
